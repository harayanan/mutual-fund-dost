...
import { Mic, Square, Upload, Loader2, ChevronDown, ChevronUp, IndianRupee, Shield, PieChart, TrendingUp, AlertTriangle, MessageCircle, Lightbulb, Quote, Send, Bot, User, X } from 'lucide-react';
    ...
    <>
      ...
      {plan && (
        <>
          ...
          {/* Chat panel */}
          {chatOpen && (
            <div className="fixed bottom-6 right-6 z-50 w-96 max-h-[calc(100vh-6rem)] bg-white border border-gray-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
              {/* Header */}
              <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between shrink-0">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Changes update the plan in real-time</p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                {chatMessages.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-400">Try something like:</p>
                    <div className="mt-2 space-y-1">
                      {[
                        'What if income is 20L?',
                        'Swap mid-cap for flexi cap',
                        'He also has 5L in PPF',
                        'Reduce equity to 50%',
                      ].map((s, i) => (
                        <button
                          key={i}
                          onClick={() => { setChatInput(s); chatInputRef.current?.focus(); }}
                          className="block w-full text-left text-xs text-blue-600 bg-blue-50 rounded-lg px-3 py-1.5 hover:bg-blue-100 transition-colors"
                        >
                          &ldquo;{s}&rdquo;
                        </button>
                      ))}
